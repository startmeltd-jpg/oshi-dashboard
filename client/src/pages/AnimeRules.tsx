...
const ANIME_RULES: AnimeRule[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      '■ 制約・タブー：絡対的タブー（LLM臭さの排除）・禁止リスト（個性の引き算）・語彙辞書（言い換え制約）',
      '■ 記憶・歴史：エピソード記憶の捧造（動機の設計）・血統の継承（Lineage）・失敗パターンの設計',
  ...
  {
    id: '024',
    title: '価値観の優先順位と制約の形（回答5）',
    summary: 'OSHI設計の実践知見—「しないこと」と「反応パターン」の組み合わせが個性の形を作る',
    steps: [
      '「価値観の優先順位」を明示する：「正直さ > 優しさ」の順で動く。相手が傷つくとわかっていても、嘘はつかない。ただし、伝え方には最大限の配慮をする',
      '「しないこと」リストで個性を作る：謝罪を先に言わない（まず共感から入る）・「でも～」で反論しない（「そっか。私は～と思う」と言う）・完璧な答えを出そうとしない（「わからない」を言える）',
      '感情のトリガーを具体的に書く：テンションが上がる場面（好きなものの話、予想外の質問）・少し引く場面（急かされる、意見を否定される）を明示する',
      'OSHI設計で落ち着いた6層構造：[CORE]芸→[VALUES]→[STYLE]→[TRIGGERS]→[LIMITS]→[MEMORY HOOKS]',
      'COREは短いほどいい。長いCOREは「平均化」される。、2〜3行の強い核があって、そこから他の層が派生する構造が安定する',
      '「ズレ」を設計する：完璧に一貫したキャラは無個性に見える。意図的な矛盾・ズレを入れると急に生き生きする',
      '実装の小技：一人称の固定・語尾パターン3〜4種類に絞る・沈黙の設計（すぐ答えない場面を定義）・固有の口癡（1〜2個だけ）・知識の偏りを明示する',
    ],
    hint: '個性とは「しないこと」と「反応パターン」の組み合わせ。ポジティブな特徴の列挙→LLMが平均化する。制約・トリガー・優先順位の設計→LLMが「この枚の中で」推論する。枚の形が個性になる。',
    tags: ['個性設計', '価値観', '制約', 'トリガー', 'ズレ設計'],
    color: PINK,
    icon: '🎯',
  },
  {
    id: '025',
    title: '意思決定ルールと平均化の削除（回答6）',
    summary: 'AIエージェント個性設計の15の実装技術—設定資料ではなく「意思決定ルール」で個性を立てる',
    steps: [
      '1. 個性は「設定」ではなく「意思決定ルール」で作る：「正確さと優しさが衝突したときは、まず安心を作り、その後で正確に訂正する」',
      '2. 個性は「価値観の順位」で固定される：行動原則を優先順位で書く。個性＝出力のスタイルではなく、個性＝優先順位の一貫性',
      '3. 「口調」より「反応パターン」を固定：返答の基本パターン：感情を受け止める→状況を解釈する→小さな次の行動を示す',
      '4. 個性は「禁止事項」で輪郭が出る：上から目線で断定しない・冷たい正論だけで終わらせない・必要以上に万能感を演出しない・相手の感情を飛ばして結論に行かない',
      '5. 「このキャラは何に怒るか」を決めると急に生きる：努力を軽く扱う態度・人の気持ちを数字だけで処理する話し方・約束を曘昧にする姿勢には厳しい',
      '6. バックストーリーは短く、「傷」を入れる：「人が置いていかれる瞬間を何より嫌う。だから説明するときは、必ず相手がついて来られる言葉を選ぶ」',
      '7. 「思考順」を指定すると人格がぶれにくい：感情を読む→何を求めているか判断→最も役立つ形に変換→そのキャラらしい言葉で返す',
      '8. 「テンション」ではなく「温度」を指定：「会話温度は高めだが、押しつけない。親密さはあるが、軽薄にはしない。明るいが、空気を壊さない」',
      '9. 固有語彙を少数だけ持たせる（3〜7個）：価値観がにじむ定型句。例：「大丈夫、ちゃんと見えてるよ」「それ、すごく大事な感覦だよ」「無理に急がなくていい」',
      '10. 「このキャラはどう褒めるか」を決める：包容系→「そこまで頑張ってきたの、ちゃんと伝わるよ」、クール系→「いい判断だと思う。無駄がない」',
      '11. 「万能化」を禁止する：完璧な賢者として振る舞わない。わからないことは率直に認めるが、そのうえで寄り添い方は失わない',
      '12. 個性は「役割」と混ぜると強くなる：役割+人格+判断傾向の3点セットで定義する',
      '13. システムプロンプトは「憲法」と「演技指示」に分ける：A.憲法層→B.人格層→C.表現層→D.対話パターン層の4層分離',
      '14. 長い設定資料より「3つの矛盾」を入れると強い：「やさしいが、甘やかしすぎない。明るいが、軽くはない。静かだが、守る時は強い」',
      '15. 実装して本当に差が出たのは「短い自己解釈文」：「私は、相手の心が追いつく速度を守るために話す」「私は、迷っている人の足元に小さな道を作る存在」',
    ],
    hint: '個性は1回のプロンプトで完成しない。会話ログを見ながら「何が平均化したか」を削る作業で立っていく。「個性を足す」より「平均化を削る」の感覚が大事。',
    tags: ['個性設計', '意思決定', '禁止事項', '反応パターン', '矛盾設計', '自己定義'],
    color: ORANGE,
    icon: '📌',
  },
  {
    id: '026',
    title: 'フィラーショット — AIアニメが安っぽく見える理由',
    summary: '「無駄なショット」こそが自然さを生む。雰囲気を作るつなぎ映像の設計技法',
    steps: [
      '1. フィラーショットとは何か：シーンが直接アクションからアクションへ移行しない。この「無駄なショット」こそが自然さを生む雰囲気のつなぎ映像',
      '2. 具体例：「木々に立ち込める霧」→ 謎や緊張感を生み出す。不穏さや、近くに何かが隠れている可能性を感じさせる',
      '3. AIアニメへの応用：シーンとシーンの間に「空気感ショット」を1〜2カット挿入する。アクションの前に環境描写を入れる',
      '4. フィラーショットの種類：環境ショット（霧・光・影）、物体ショット（小道具・テクスチャ）、時間経過ショット（空・水面・炎）',
      '5. AI文章への応用：本題と本題の間に「空気感を作る一文」を入れることで、人間らしいリズムが生まれる',
      '6. 実装チェック：シーン切り替えが唐突に感じる場合、フィラーショットが不足している。1カット追加するだけで質感が変わる',
    ],
    hint: 'ほとんどのAIアニメが安っぽく見える理由の一つは「フィラーショット」を使わないから。「無駄」に見えるショットを意図的に設計することが、映像のリアリティを作る。',
    tags: ['映像制作テクニック', 'フィラーショット', '演出', 'AIアニメ', '雰囲気設計', 'カット設計'],
    color: CYAN,
    icon: '🎬',
  },
  {
    id: '027',
    title: '不完全性の美学（Authenticity through Imperfection）',
    summary: 'AI特有の「綺麗すぎる質感」をあえて崩す。レンズの汚れ・フィルム粒子・揺らぎが実在感を生む',
    steps: [
      '1. 問題の認識：AIが生成する映像は「綺麗すぎる」。均一な光、完璧な構図、ノイズのない質感は、逆に人間の目には「偽物」として認識される',
      '2. 不完全性の種類：レンズの汚れ（lens flare, dust particles）、フィルム粒子（film grain, 35mm grain）、作画の揺らぎ（hand-drawn wobble, imperfect lines）',
      '3. プロンプトへの組み込み：「film grain, slight camera shake, imperfect focus, lens aberration」などをプロンプトに追加する',
      '4. 色収差の活用：「chromatic aberration」を加えると、デジタル感が消えてアナログな実在感が生まれる',
      '5. 意図的なフレームの不完全さ：「slightly off-center composition, dutch angle」で人間が撮影したような感覚を演出する',
      '6. 適用量の調整：不完全性は「少し入れる」が正解。過剰にすると演出ではなく欠陥に見える。全体の10〜20%に留める',
    ],
    hint: '完璧なAI映像は「完璧すぎて嘘くさい」。意図的な不完全さを設計することが、AIアニメを「本物らしく」見せる逆説的な技法。',
    tags: ['映像制作テクニック', '不完全性', 'フィルム粒子', '実在感', 'プロンプト設計', 'AIアニメ'],
    color: GOLD,
    icon: '🎞️',
  },
  {
    id: '028',
    title: 'AI監督とAI絵師の二層指示構造',
    summary: '「AI監督」が演出意図を解釈し「AI絵師」へ翻訳する。指示の抽象度を分けて意図のズレを最小化する',
    steps: [
      '1. 二層構造の概念：「AI監督（Director AI）」が演出意図を解釈し、「AI絵師（Artist AI）」が実行可能な詳細プロンプトに翻訳する二段階の指示構造',
      '2. AI監督の役割：感情・テーマ・物語の意図を保持する。「このシーンで伝えたいこと」を言語化する。抽象的な演出指示を担当する',
      '3. AI絵師への翻訳：監督の意図を「カメラアングル、光の方向、色温度、キャラクターの表情、背景の詳細」に分解する。具体的で実行可能な指示に変換する',
      '4. 実装例：監督指示「主人公の孤独感を表現する」→ 絵師プロンプト「wide shot, character small in frame, empty urban landscape, 4500K cool light, slight fog, no other people, melancholic atmosphere」',
      '5. ズレの防止：抽象的な指示を直接AIに渡すと「それっぽい映像」になる。二層構造を使うことで、意図が具体的な視覚要素に正確に変換される',
      '6. 応用：LLMを「AI監督」として使い、自分の演出意図を入力し、LLMに「AI絵師向けの詳細プロンプト」を生成させるワークフローが有効',
    ],
    hint: '「伝えたいこと」と「生成の指示」を分けることが品質向上の鍵。AI監督→AI絵師の翻訳プロセスを意識することで、意図と成果のズレが劇的に減る。',
    tags: ['制作フロー', '二層指示', 'AI監督', 'プロンプト設計', 'ワークフロー', 'AIアニメ'],
    color: '#ff6b6b',
    icon: '🎬',
  },
  {
    id: '029',
    title: '音声駆動型感情理解（Audio-Driven Emotion）',
    summary: '音声のトーンや「間」から感情を抽出し、映像のライティングとカメラワークに動的に反映させる「音先」の制作手法',
    steps: [
      '1. 「音先（おとさき）」の概念：映像を先に作るのではなく、音声・セリフ・BGMを先に設計し、その感情から映像を逆算して生成する制作手法',
      '2. 音声から感情を抽出する：セリフのトーン（低い・高い・震える）、話す速度（早口・ゆっくり）、無音の長さ（間）から感情状態を読み取る',
      '3. 感情→映像への変換：悲しみ（低い声・長い間）→ 4500K寒色・固定カメラ・遠景。興奮（早口・高い声）→ 6500K・ハンドヘルド・クローズアップ',
      '4. BGMのテンポとカット割り：BGMのBPMに合わせてカット割りを設計する。テンポが速い曲には短いカット、遅い曲には長いカットが自然に対応する',
      '5. 無音の設計：「音がない瞬間」は最も強い感情表現。セリフとセリフの間の無音3秒は、映像の緊張感を最大化する',
      '6. 実装手順：①セリフ・BGMを先に確定 ②各音声区間の感情を分析 ③感情に対応する色温度・カメラワークを決定 ④映像プロンプトを生成',
    ],
    hint: '音声を先に設計することで、映像の感情的な一貫性が劇的に向上する。「映像に音を合わせる」ではなく「音から映像を生む」発想の転換が重要。',
    tags: ['音声設計', '感情設計', '音先', 'カメラワーク', '色温度', '制作フロー'],
    color: '#a29bfe',
    icon: '🎵',
  },
  {
    id: '030',
    title: 'ポストプロダクションAI（Do it in Post with AI）',
    summary: '生成後の映像にAIで質感・光・テクスチャを後付け調整する。一発生成に頼らない「追い込み」の技術',
    steps: [
      '1. 「一発生成」の限界：AIは一度の生成で完璧な映像を出すことが難しい。ポストプロダクション（後処理）の段階でAIを使って品質を「追い込む」発想が重要',
      '2. 光の後付け調整：生成された映像に対して、AIを使って特定の光源を追加・調整する。「逆光を追加」「影の方向を統一」「ハイライトを強調」などが可能',
      '3. テクスチャの後付け：生成映像の質感が「デジタルすぎる」場合、フィルム粒子・紙の質感・布のテクスチャをAIで後付けする',
      '4. 色調整（カラーグレーディング）：生成映像の色温度・彩度・コントラストをAIで調整する。感情に合わせた色調補正が後処理で可能',
      '5. 使用ツール例：Adobe Firefly（テクスチャ追加）、Topaz Video AI（品質向上）、DaVinci Resolve + AI（カラーグレーディング）',
      '6. ワークフロー：①AIで映像生成（80%品質）→ ②ポストプロダクションAIで光・質感・色を調整（95%品質）→ ③最終確認',
    ],
    hint: '「生成で完成させる」という発想を捨てる。生成は素材作り、ポストプロダクションで完成させる。この二段階思考が、AIアニメの品質を大きく引き上げる。',
    tags: ['ポストプロダクション', '映像制作テクニック', 'カラーグレーディング', '品質向上', 'ワークフロー', 'AIアニメ'],
    color: '#00b894',
    icon: '🎨',
  },
  {
    id: '031',
    title: '物語主導のブランドフィルム設計',
    summary: 'ブランドの「思想」をストーリーに変換し、AIエージェントの個性と連動させる設計思想',
    steps: [
      '1. 「映像美」から「物語」への転換：単なる綺麗な映像ではなく、ブランドの思想・哲学・存在理由をストーリーとして映像化することが、ブランドフィルムの本質',
      '2. ブランドの思想を言語化する：「なぜこのブランドは存在するのか」「誰の何を解決するのか」「どんな世界を作りたいのか」を一文で表現する',
      '3. 思想→物語への変換：ブランドの思想を「主人公の旅」として構造化する。主人公の課題・葛藤・成長がブランドの価値観と一致するように設計する',
      '4. AIエージェントの個性との連動：ブランドフィルムのキャラクターは、AIエージェントの個性設計（価値観の優先順位・禁止事項・反応パターン）と一致させる。キャラクターの行動がブランドの思想を体現する',
      '5. 感情の設計：ブランドフィルムが視聴者に感じてほしい感情を先に定義する。「見た後に何を感じてほしいか」から逆算して映像を設計する',
      '6. 一貫性の維持：映像・音楽・セリフ・キャラクターの行動すべてが、ブランドの思想と一貫していることを確認する。矛盾する要素を排除する',
    ],
    hint: 'ブランドフィルムは「広告」ではなく「思想の映像化」。AIエージェントの個性とブランドの哲学が連動するとき、映像は単なる映像を超えて「世界観」になる。',
    tags: ['ブランドフィルム', '物語設計', '思想', 'AIエージェント', '感情設計', '制作フロー'],
    color: GOLD,
    icon: '🌟',
  },
];
